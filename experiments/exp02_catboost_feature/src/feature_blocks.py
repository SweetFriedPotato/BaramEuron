...
# ==============================================================================
# 신규 추가: 초안 기반 대기안정도, 난류 및 악천후(착빙) 피처 블록
# ==============================================================================

def add_advanced_meteorology_features(features: pd.DataFrame, group_id: int, eps: float = EPS) -> pd.DataFrame:
    """초안 반영: 행성경계층(PBLH), 복사량, 난류 강도, 악천후(착빙) 및 풍력 공식 상호작용 피처 추가."""
    out = features.copy()
    group_id = int(group_id)
    
    # [1] 지표면 돌풍(Gust) 변동성을 활용한 난류 강도(Turbulence Intensity) 추정
    if "gfs__gust__mean" in out and "gfs__ws10__mean" in out:
        out["gfs__turbulence_intensity_10m"] = (out["gfs__gust__mean"] - out["gfs__ws10__mean"]) / (out["gfs__ws10__mean"] + eps)
        out["gfs__gust_factor"] = out["gfs__gust__mean"] / (out["gfs__ws10__mean"] + eps)

    # [2] 대기 안정도 및 연직 혼합도 (복사량 및 운량 활용)
    for kind in ("ldaps", "gfs"):
        rad_short = f"{kind}__shortwave_radiation__mean"
        rad_long = f"{kind}__longwave_radiation__mean"
        tcc = f"{kind}__total_cloud_cover__mean"
        
        if all(col in out for col in (rad_short, rad_long)):
            out[f"{kind}__net_radiation"] = out[rad_short] + out[rad_long]
            if tcc in out:
                out[f"{kind}__radiation_cloud_interaction"] = out[f"{kind}__net_radiation"] * (1.0 - out[tcc] / 100.0)

    # [3] 행성 경계층(PBL) 높이와 연직 혼합도
    if "gfs__hpbl__mean" in out and "gfs__ws100__mean" in out and "gfs__ws10__mean" in out:
        out["gfs__vertical_shear_per_pblh"] = (out["gfs__ws100__mean"] - out["gfs__ws10__mean"]) / (out["gfs__hpbl__mean"] + eps)

    # [4] 악천후 및 착빙(Icing) 위험도 탐지 변수
    for kind in ("ldaps", "gfs"):
        temp_k = f"{kind}__temperature_2m__mean"
        rh = f"{kind}__relative_humidity_2m__mean"
        precip = f"{kind}__precipitation_rate__mean"
        lcc = f"{kind}__low_cloud_cover__mean"
        
        if temp_k in out:
            temp_c = out[temp_k] - 273.15
            is_freezing = (temp_c <= 0.0).astype(float)
            
            if rh in out:
                out[f"{kind}__icing_risk_humidity"] = is_freezing * (out[rh] / 100.0)
            if lcc in out:
                out[f"{kind}__icing_risk_low_cloud"] = is_freezing * (out[lcc] / 100.0)
            if precip in out:
                out[f"{kind}__icing_risk_precipitation"] = is_freezing * out[precip]

    # ==============================================================================
    # [5] 신규 추가: 풍력 에너지 이론 공식 모사 변수 (P ∝ rho * v^3)
    # ==============================================================================
    # 전력 공식의 A(로터 수광면적)와 계수 1/2은 상수이므로, 머신러닝 모델이 스케일링을 학습하도록 
    # 변수 수준에서는 핵심 물리적 변동 원인인 '밀도 * 풍속^3'만 정밀하게 결합합니다.
    
    # GFS 허브 높이(117m) 풍속 기반 운동에너지 잠재량
    gfs_ws117 = "gfs__ws117_from_80_100__mean"
    gfs_rho = "gfs__thermo____mean__moist_air_density_kg_m3"
    if gfs_ws117 in out and gfs_rho in out:
        out["gfs__kinetic_energy_potential_hub117"] = out[gfs_rho] * (out[gfs_ws117] ** 3)

    # LDAPS 허브 높이(117m) 풍속 기반 운동에너지 잠재량
    ldaps_ws117 = "ldaps__ws117_from_10_50__mean"
    ldaps_rho = "ldaps__thermo____mean__moist_air_density_kg_m3"
    if ldaps_ws117 in out and ldaps_rho in out:
        out["ldaps__kinetic_energy_potential_hub117"] = out[ldaps_rho] * (out[ldaps_ws117] ** 3)
...
class FeatureBlockPipeline:
    ...
    def transform(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        # 초안 반영 신규 블록 작동 트리거 추가
        if self.blocks.get("advanced_meteorology", False):
            out = add_advanced_meteorology_features(out, self.group_id)
